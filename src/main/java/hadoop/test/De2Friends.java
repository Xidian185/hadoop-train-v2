...
public class De2Friends {
    ...
    public static void main(String[] args) throws Exception{
        ...
        conf.set("fs.defaultFS","hdfs://192.168.43.26:8020");
        System.setProperty("HADOOP_USER_NAME","hadoop");
        System.setProperty("hadoop.home.dir", "E:\\BaiduNetdiskDownload\\新建文件夹\\hadoop-2.6.0-cdh5.15.1");
        conf.set("mapreduce.app-submission.cross-platform", "true");
        Path fileInput = new Path("/qq/qq.txt");
        Path tempDir = new Path("/mycluster/output/deg2friend-temp-" + Integer.toString(new Random().nextInt(Integer.MAX_VALUE)));
        Path fileOutput = new Path("/mycluster/output/qq");

        FileSystem fileSystem = FileSystem.get(new URI("hdfs://192.168.43.26:8020"),conf,"hadoop");
        //Path path = new Path("/access/output");
        if(fileSystem.exists(fileOutput)){
            fileSystem.delete(fileOutput, true);
        }
